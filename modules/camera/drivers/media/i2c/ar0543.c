...
extern void gsensor_info_for_camera(int *info_x, int *info_y, int *info_z);
#endif
...
int ar0543_read_otp_probe(struct i2c_client *client);
...
static int ar0543_s_power(struct v4l2_subdev *sd, int power);
...
BB_WrSPIFlash(struct v4l2_subdev *sd, char* binfile_path, bool erase_all)
{
	...
	if (ret) {
		dev_err(&client->dev, "ar0543 power-up err");
	}
	...
	if (ret) {
		dev_err(&client->dev, "ar0543 power down err");
	}	
...
static ssize_t i7002a_proc_read(char *page, char **start, off_t off, int count, 
            	int *eof, void *data)
{
//FW_BSP++
	struct file *fp = NULL;
...
//FW_BSP--	
	...
	if (SPI_ENABLE==0) {
		return sprintf(page, "version_in_isp:%x\n", version_num_in_isp);
	} else {
		return sprintf(page, "version_in_bin:%x\n", version_num_in_bin);
	}
}
...
{
    i7002a_proc_file = create_proc_entry(i7002a_PROC_FILE, 0666, NULL);
    if (i7002a_proc_file) {
		i7002a_proc_file->read_proc = i7002a_proc_read;
...
int ar0543_read_otp_probe(struct i2c_client *client)
{
    ...
    //Decide OTP bank. Sensor vendor will write bank 0 first.
    //If bank 0 is failed, vendor will write bank 1 and so on.
    //We have to check from bank 2 to bank 0 to ensure the values we read are correct.

    //Aptina OTP value is placed from bank 0 to bank 2.
    ...
    if(((read_value[0] == 0) && (read_value[1] == 0)) || ((read_value[0] == 0xff) && (read_value[1] == 0xff))) {
        //Bank 2 is null, check bank 1
        sensor_write_reg(client, 0x718B, 0x10);
        msleep(5);
        sensor_read_reg(client, 0x72A6, &read_value[0]);
        sensor_write_reg(client, 0x718B, 0x11);
        msleep(5);
        sensor_read_reg(client, 0x72A6, &read_value[1]);
        pr_info("%s Check bank 1 0x%X 0x%X\n",__func__,read_value[0],read_value[1]);
        if(((read_value[0] == 0) && (read_value[1] == 0)) || ((read_value[0] == 0xff) && (read_value[1] == 0xff))) {
            //Bank 1 is null, read bank 0
            pr_info("%s Read bank 0\n",__func__);
            for (i=0; i<10; i++) {
                sensor_write_reg(client, 0x718B, i);
                msleep(5);
                sensor_read_reg(client, 0x72A6, &read_value[i]);
                //pr_info("%s Read bank 0, byte %d 0x%X\n",__func__, i, read_value[i]);
            }
        }else{
            //Read bank 1
...
int sensor_s_roi_trigger(struct v4l2_subdev *sd, int trigger)
{
	...
	gsensor_info_for_camera(&gsensor_x, &gsensor_y, &gsensor_z);
...
static int ar0543_init_common(struct v4l2_subdev *sd)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int ret;
//FW_BSP++
...
//FW_BSP++
	...
	if (FIRST_BOOTING==true) {
		
		msleep(60);

		if (PROJECT_ID==PROJ_ID_ME372CG) {
			ret = gpio_request(GP_CORE_014, GP_SENSOR_2_8V);
			if (ret) {
				printk("%s: failed to request gpio(pin %d)\n", __func__, GP_CORE_014);
			} else {
				gpio_direction_output(GP_CORE_014, 0);
				gpio_set_value(GP_CORE_014, 1);
				gpio_free(GP_CORE_014);
		...
		sensor_write_reg(client, 0x9033, 0x10);
		...
		sensor_write_reg(client, 0x9032, 0x10);
		mdelay(10);
		...
		// Start - I2C Read
		sensor_write_reg(client, 0x923C, 0x01);	// Sub address enable
		sensor_write_reg(client, 0x9238, 0x30);	// Sub address enable
		sensor_write_reg(client, 0x9240, 0x6C);	// Slave address
		sensor_write_reg(client, 0x9200, 0x03);	// Read mode
		sensor_write_reg(client, 0x9210, 0x00);	// Register addr MSB
		sensor_write_reg(client, 0x9212, 0x00);	// Register addr LSB
		sensor_write_reg(client, 0x9204, 0x01);	// Trigger I2C read
		mdelay(5);
		sensor_read_reg(client, 0x9211, &retvalue_h);
		sensor_read_reg(client, 0x9213, &retvalue_l);
...
static int ar0543_s_power(struct v4l2_subdev *sd, int power)
{
	...
	return ar0543_init_common(sd);
}

static int ar0543_try_res(u32 *w, u32 *h)
{
	int i;
	 ...
	 */
	for (i = 0; i < N_RES; i++) {
		if ((ar0543_res[i].width >= *w) &&
		    (ar0543_res[i].height >= *h))
			break;
	}
	...
	*w = ar0543_res[i].width;
	*h = ar0543_res[i].height;

	return 0;
}

static struct ar0543_res_struct *ar0543_to_res(u32 w, u32 h)
{
	int  index;
	...
	for (index = 0; index < N_RES; index++) {
		if ((ar0543_res[index].width == w) &&
		    (ar0543_res[index].height == h))
			break;
	}
	...
	return &ar0543_res[index];
}

static int ar0543_try_mbus_fmt(struct v4l2_subdev *sd,
				struct v4l2_mbus_framefmt *fmt)
{
	return ar0543_try_res(&fmt->width, &fmt->height);
}

static int ar0543_res2size(unsigned int res, int *h_size, int *v_size)
{
	...
	switch (res) {
	case AR0543_RES_QCIF:
		hsize = AR0543_RES_QCIF_SIZE_H;
		vsize = AR0543_RES_QCIF_SIZE_V;
		break;
	case AR0543_RES_QVGA:
		hsize = AR0543_RES_QVGA_SIZE_H;
		vsize = AR0543_RES_QVGA_SIZE_V;
		break;
	case AR0543_RES_CIF:
		hsize = AR0543_RES_CIF_SIZE_H;
		vsize = AR0543_RES_CIF_SIZE_V;
		break;
	case AR0543_RES_VGA:
		hsize = AR0543_RES_VGA_SIZE_H;
		vsize = AR0543_RES_VGA_SIZE_V;
		break;
/*
	case AR0543_RES_480P:
		hsize = AR0543_RES_480P_SIZE_H;
		vsize = AR0543_RES_480P_SIZE_V;
		break;
*/
	case AR0543_RES_720P:
		hsize = AR0543_RES_720P_SIZE_H;
		vsize = AR0543_RES_720P_SIZE_V;
		break;
	case AR0543_RES_960P:
		hsize = AR0543_RES_960P_SIZE_H;
		vsize = AR0543_RES_960P_SIZE_V;
		break;
	case AR0543_RES_1080P:
		hsize = AR0543_RES_1080P_SIZE_H;
		vsize = AR0543_RES_1080P_SIZE_V;
		break;
	case AR0543_RES_3M:
		hsize = AR0543_RES_3M_SIZE_H;
		vsize = AR0543_RES_3M_SIZE_V;
		break;
	case AR0543_RES_5M:
		hsize = AR0543_RES_5M_SIZE_H;
		vsize = AR0543_RES_5M_SIZE_V;
		break;
	default:
...
static int ar0543_get_mbus_fmt(struct v4l2_subdev *sd,
				struct v4l2_mbus_framefmt *fmt)
{
	struct ar0543_device *dev = to_ar0543_sensor(sd);
	int width, height;
	...
	ret = ar0543_res2size(dev->res, &width, &height);
	if (ret)
		return ret;
	fmt->width = width;
	fmt->height = height;
...
static int ar0543_set_mbus_fmt(struct v4l2_subdev *sd,
			      struct v4l2_mbus_framefmt *fmt)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct ar0543_device *dev = to_ar0543_sensor(sd);
	struct ar0543_res_struct *res_index;
	u32 width = fmt->width;
	...
	ar0543_try_res(&width, &height);
	res_index = ar0543_to_res(width, height);
	...
	if(hdr_enable && sensor_mode == ATOMISP_RUN_MODE_STILL_CAPTURE){
		...
		sensor_write_reg(client, 0x7124, 0x0C);  //Fix frame rate
		...
		case AR0543_RES_1080P:
			...
			break;
		case AR0543_RES_3M:
			sensor_write_reg(client, 0x7108, 0x12); //SET resolution 2048x1536
		...
		/*
		case AR0543_RES_QCIF:
			ret = ar0543_write_reg_array(c, ar0543_qcif_30_init);
			dev_info(&c->dev, "%s: set for qcif\n", __func__);
			break;
		case AR0543_RES_QVGA:
			ret = ar0543_write_reg_array(c, ar0543_qvga_30_init);
		...
		*/
		...
		/*
		case AR0543_RES_480P:
			ret = ar0543_write_reg_array(c, ar0543_svga_30_init);
		...
		*/
		...
		case AR0543_RES_1080P:
			...
			break;
		case AR0543_RES_3M:
			sensor_write_reg(client, 0x7106, 0x12); //SET resolution 2048x1536
			dev_info(&client->dev, "%s: Snapshot 3M\n", __func__);
			break;
		case AR0543_RES_5M:
 ...
 * This returns EV.
 */
static int ar0543_get_exposure_bias(struct v4l2_subdev *sd, s32 *value)
{
	*value = 0;
...
static struct ar0543_control ar0543_controls[] = {
	{
		...
		.query = ar0543_get_exposure_bias, //For EXIF value, need to combine with other parameter
	},
	{
		.qc = {
...
static struct ar0543_control *ar0543_find_control(__u32 id)
{
	int i;
...
static int ar0543_detect(struct ar0543_device *dev, struct i2c_client *client)
{
...
static int
ar0543_s_config(struct v4l2_subdev *sd, int irq, void *platform_data)
{
	struct ar0543_device *dev = to_ar0543_sensor(sd);
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int ret;
	...
	if (dev->platform_data->platform_init) {
		...
		if (ret) {
			dev_err(&client->dev, "ar0543 platform init err\n");
			return ret;
		}
	}
	ret = ar0543_s_power(sd, 1);
	if (ret) {
		dev_err(&client->dev, "ar0543 power-up err\n");
		//goto fail_detect;
	}
	...
	ret = ar0543_detect(dev, client);
	if (ret) {
		dev_err(&client->dev, "ar0543_detect err s_config.\n");
		//goto fail_detect;
	}
...
static int ar0543_s_ctrl_old(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct ar0543_control *octrl = ar0543_find_control(ctrl->id);
	int ret;
...
static int
ar0543_enum_framesizes(struct v4l2_subdev *sd, struct v4l2_frmsizeenum *fsize)
{
				       ...
				       struct v4l2_frmivalenum *fival)
{
	...
	for (i = 0; i < N_RES; i++) {
		if ((ar0543_res[i].width >= fival->width) &&
		    (ar0543_res[i].height >= fival->height))
			break;
	}
	if (i == N_RES)
		i--;
...
static int
ar0543_g_chip_ident(struct v4l2_subdev *sd, struct v4l2_dbg_chip_ident *chip)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
...
static int ar0543_enum_mbus_code(struct v4l2_subdev *sd,
...
static int ar0543_enum_frame_size(struct v4l2_subdev *sd,
	struct v4l2_subdev_fh *fh,
...
{
	struct ar0543_device *snr = to_ar0543_sensor(sd);
	struct v4l2_mbus_framefmt *format =
			__ar0543_get_pad_format(snr, fh, fmt->pad, fmt->which);
...
static int ar0543_s_ctrl(struct v4l2_ctrl *ctrl)
{
	struct ar0543_device *snr = container_of(
		ctrl->handler, struct ar0543_device, ctrl_handler);
...
static int ar0543_g_skip_frames(struct v4l2_subdev *sd, u32 *frames)
{
	...
	for (index = 0; index < N_RES; index++) {
		if (ar0543_res[index].res == snr->res)
			break;
	}
	...
	*frames = ar0543_res[index].skip_frames;

	return 0;
}

static int ar0543_s_modes(struct v4l2_subdev *sd, u32 modes)
{
	struct ar0543_device *snr = to_ar0543_sensor(sd);
...
static const struct v4l2_subdev_video_ops ar0543_video_ops = {
	...
	.s_mbus_fmt = ar0543_set_mbus_fmt,
	.g_mbus_fmt = ar0543_get_mbus_fmt,
...
static int ar0543_remove(struct i2c_client *client)
{
	...
	dev = container_of(sd, struct ar0543_device, sd);
	dev->platform_data->csi_cfg(sd, 0);
