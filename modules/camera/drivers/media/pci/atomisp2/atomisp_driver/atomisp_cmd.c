...
#if defined(CONFIG_ME175CG) || defined(CONFIG_A400CG) || defined(CONFIG_ME372CL)
...
//BSP+++
//	if (IS_BYT) {
	if(!(strncmp(isp->inputs[asd->input_curr].camera->name,"hm2056_raw",10))){			
		padding_w = 12;
		padding_h = 12;
	}
//BSP---
	...
	if (isp_sink_crop.width * 9 / 10 < f->fmt.pix.width ||
		...
		//Intel Patch+++
		/* Crop if we have different required aspect ratio from sensor output
		 * multiply by 10 to have more precious result */
		if ((main_compose.width*10/main_compose.height) != (isp_sink_crop.width*10/isp_sink_crop.height))
		{
			atomisp_subdev_set_selection(&asd->subdev, NULL,
					     V4L2_SUBDEV_FORMAT_ACTIVE,
					     ATOMISP_SUBDEV_PAD_SINK,
