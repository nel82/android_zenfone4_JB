...
stopsensor:
	...
	if (isp->sw_contex.power_state == ATOM_ISP_POWER_UP) {
		if (isp->isp_timeout)
			dev_err(isp->dev, "%s: Resetting with WA activated", __func__);
		atomisp_reset(isp);
	...
	struct v4l2_control *control)
{
	...
	struct atomisp_sub_device *asd = atomisp_to_video_pipe(vdev)->asd;
	struct atomisp_device *isp = video_get_drvdata(vdev);
	struct v4l2_control ctrl; //ASUS_BSP++, for calibration
	int i, ret = -EINVAL;
	...
	//ASUS_BSP+++
	case V4L2_CID_AF_RESULT:
	case V4L2_CID_EDGE:
	case V4L2_CID_Y_INFO:
	//for iCatch 3A information+++
	case V4L2_CID_ICATCH_3A_AE1: //AE1
	case V4L2_CID_ICATCH_3A_AE2: //AE2
	case V4L2_CID_ICATCH_3A_AWB1: //AWB1
	case V4L2_CID_ICATCH_3A_AWB2: //AWB2 and 3A version
	case V4L2_CID_ICATCH_3A_AF1: //AF1
	case V4L2_CID_ICATCH_3A_AF2: //AF2
	//for iCatch 3A information---
	//for calibration+++
	case CUSTOM_IOCTL_REG_GET:	
	case CUSTOM_IOCTL_SPI_GET:
		mutex_unlock(&isp->mutex);
		//printk("id:0x%x, val:0x%x\n", control->id, control->value);
		ret = v4l2_subdev_call(isp->inputs[asd->input_curr].camera,
				       core, g_ctrl, &ctrl);
		control->value = ctrl.value;
		//printk("After val:0x%x\n", control->value);
		return ret;
	//for calibration---
	//ASUS_BSP---
			  ...
			  struct v4l2_control *control)
{
	...
	//ASUS_BSP+++
	case V4L2_CID_COLORFX:
	case CUSTOM_IOCTL_REG_SET: //for calibration
	case V4L2_CID_EFFECT_AURA:
	case V4L2_CID_3A_LOCK_ISP:
	case V4L2_CID_WDR:
	case V4L2_CID_HDR:
	case V4L2_CID_PRIVIEW_EXPOSURE_TIME:
	case V4L2_CID_ROI_TRIGGER:
	case V4L2_CID_AF_WINDOW:
	//ASUS_BSP---
		mutex_unlock(&isp->mutex);
	...
	struct v4l2_ext_controls *c)
{
	...
	for (i = 0; i < c->count; i++) {
		...
		case V4L2_CID_FOCUS_STATUS:
		case V4L2_CID_FOCUS_AUTO:
		case V4L2_CID_AUTO_FOCUS_RANGE: //ASUS_BSP+++
		case V4L2_CID_AF_WINDOW: //ASUS_BSP+++
			if (isp->inputs[asd->input_curr].motor)
				ret = v4l2_subdev_call(
					isp->inputs[asd->input_curr].motor,
					core, s_ctrl, &ctrl);
	...
	struct v4l2_streamparm *parm)
{
	...
	struct atomisp_sub_device *asd = atomisp_to_video_pipe(vdev)->asd;
	int mode;
	int rval, ret;  //ASUS to set modes++ 
	...
	//ASUS to set modes++ 
	ret = v4l2_subdev_call(isp->inputs[asd->input_curr].camera,
					sensor, s_modes, (u32 )mode);

	if(ret == -EINVAL)
		dev_err(isp->dev, "unsupport type mode\n");
	//ASUS to set modes-- 
