			...
			struct intel_sprite_context *psContext, u32 *pipe_mask)
{
	...
	if ((psContext->update_mask & SPRITE_UPDATE_SIZE)) {
		...
		if(panel_id == 0)
			PSB_WVDC32((psContext->size > 0x31f01df)? 0x31f01df : psContext->size, DSPASIZE + reg_offset);
		else
			...
			struct intel_sprite_context *psContext, u32 *pipe_mask)
{
	...
	if ((psContext->update_mask & SPRITE_UPDATE_SIZE)) {
#ifdef CONFIG_SUPPORT_DDS_MIPI_SWITCH
		PSB_WVDC32((panel_id==0)?0x31f01df:0x4ff031f, DSPASIZE + reg_offset);
