		...
		PVRSRV_KERNEL_MEM_INFO **ppsKernelMemInfo)
{
	...
	if (psPerProc == NULL) {
		DRM_ERROR("Cannot find PID %x from hash table for handle 0x%x\n",
			  psb_get_tgid(), (IMG_UINT32)hKernelMemInfo);
		dump_stack();
	}
	...
	if(ppsKernelMemInfo == NULL){
		printk("Display debug ppsKernelMemInfo = NULL\n");
		dump_stack();
	}
