...
static unsigned int PCB_ID = 0xFF;
static int sim_number = 1;
...
static int dlp_trace_dev_open(struct inode *inode, struct file *filp)
{
    ...
    if (!dlp_tty_is_link_valid()) {
        pr_err (DRVNAME": <Kris> Can't Open CH%d (HSI CH%d) => tty is not valid! Maybe TX timeout or Modem Crash\n", ch_ctx->ch_id, ch_ctx->hsi_channel);
        ret -ENODEV;
        goto out;
    }
	...
	if (state != DLP_CH_STATE_CLOSED) {
		...
		if (ch_ctx->hsi_channel == 4) {
			/* Kris : Workaround Set Trace Ch4 to CLOSE STATE */
    ...
    pr_err (DRVNAME" : Project ID = %d, sim_number = %d\n", PROJECT_ID, sim_number);
    if (PROJECT_ID == PROJ_ID_PF400CG || 
        PROJECT_ID == PROJ_ID_ME372CL || 
        PROJECT_ID == PROJ_ID_A400CG || 
        (PROJECT_ID == PROJ_ID_ME175CG && sim_number == 2)) {
        ...
        pr_err (DRVNAME" : Skip Open Channel\n");
    }
...
static int dlp_trace_dev_close(struct inode *inode, struct file *filp)
{
    ...
    if (!dlp_tty_is_link_valid()) {
        pr_err (DRVNAME": <Kris> modem recovery, can't close device.\n");
        return -ENODEV;
    }
			   ...
			   loff_t *ppos)
{
    ...
    if (!dlp_tty_is_link_valid()) {
        pr_err (DRVNAME": <Kris> modem recovery or power off, can't read mdmTrace now.\n");
        return -EINVAL;
    }
		...
		struct poll_table_struct *pt)
{
    ...
    if (!dlp_tty_is_link_valid()) {
        pr_err (DRVNAME": <Kris> modem recovery or power off, can't poll mdmTrace now.\n");
        goto out;
    }
		...
		struct device *dev)
{
    ...
    /* Kris : add for check Project and SKU */
    PROJECT_ID = Read_PROJ_ID ();
    PCB_ID = Read_PCB_ID ();
    sim_number = ((PCB_ID & (1 << 6)) >> 6 ) + 1;
