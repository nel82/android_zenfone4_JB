...
int dlp_hsi_controller_push(struct dlp_xfer_ctx *xfer_ctx, struct hsi_msg *pdu)
{
	...
	if (pdu->ttype == HSI_MSG_WRITE) {
		...
		if (EDLP_NET_TX_DATA_LEN_REPORT)
			pr_err(DRVNAME ": CH%d (HSI CH%d) credits=%d, seq_num=%d", 
				ch_ctx->ch_id,
				ch_ctx->hsi_channel,
				xfer_ctx->channel->credits, xfer_ctx->seq_num);
	}
	spin_unlock_irqrestore(&ch_ctx->lock, flags);
...
static void dlp_hsi_ehandler(struct hsi_client *cl, unsigned long event)
{
	...
	int i;
#if 1 // disable_rx_call_back
	int l_rx_callback_enable = atomic_read(&dlp_drv.rx_callback_enable);
	if(l_rx_callback_enable == 0)
	{
		pr_debug(DRVNAME" dlp_hsi_ehandler disable rx callback\n");
		return;
	}
#endif
...
#if 1 // disable_rx_call_back
void dlp_ctrl_disable_callbacks(void)
{
	atomic_set(&dlp_drv.rx_callback_enable, 0);
}

void dlp_ctrl_enable_callbacks(void)
{
	atomic_set(&dlp_drv.rx_callback_enable, 1);
}
#endif
