	...
	if (data && 0 == strncmp(cmd, "recovery", 9)) {
		...
		if (ret_ipc < 0)
			pr_err("%s cannot write Fastboot reboot reason in OSNIB\n",
				__func__);
		ret = NOTIFY_OK;
	} else if (data && 0 == strncmp(cmd, "factory-reset", strlen("factory-reset"))) {
		//Will add for ADB reboot reset , setting reboot reason is RECOVERY
		pr_warn("[SHTDWN] %s, rebooting into factory-reset\n", __func__);
		ret_ipc = intel_scu_ipc_write_osnib_rr(SIGNED_RECOVERY_ATTR);
		if (ret_ipc < 0)
