...
#define MDM_VSD2_DELAY 10000 /* modem sd2 drop to 0V needs 8.5 ms */
#define UART1_RTS   66
#define UART1_CTS   68
#define UART1_RX    64
#define UART1_TX    65
...
static int mdm_ctrl_cold_boot(struct mdm_ctrl *drv)
{
    ...
    /* Disable all Uart1 GPIO */
    gpio_direction_output (UART1_RTS, 0);
    gpio_direction_output (UART1_CTS, 0);
    gpio_direction_output (UART1_RX, 0);
    gpio_direction_output (UART1_TX, 0);
    usleep_range (MDM_VSD2_DELAY, MDM_VSD2_DELAY);
    ...
    /* Set Uart1 to disable State */
    /* add a 10ms delay for wait SD2 */
    usleep_range (MDM_VSD2_DELAY, MDM_VSD2_DELAY);
    lnw_gpio_set_alt(UART1_RX, LNW_GPIO);
    gpio_direction_input(UART1_RX);
    gpio_direction_output (UART1_TX, 1);
    lnw_gpio_set_alt (UART1_TX, LNW_GPIO);
    usleep_range (10, 10);
    gpio_direction_input (UART1_TX);
    lnw_gpio_set_alt (UART1_CTS, LNW_GPIO);
    gpio_direction_input (UART1_CTS);
    lnw_gpio_set_alt (UART1_RTS, LNW_GPIO);
    gpio_direction_input (UART1_RTS);
